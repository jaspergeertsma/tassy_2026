...
function parseReisHTML(htmlContent: string): ItineraryDay[] {
...
function mergeItineraries(newDays: ItineraryDay[], existingDays: ItineraryDay[]): ItineraryDay[] {
    const merged: ItineraryDay[] = [];

    // Create a map of existing days by title for easy lookup
    const existingMap = new Map<string, ItineraryDay>();
...
async function main() {
    try {
        ...
        const existingItinerary: ItineraryDay[] = JSON.parse(
